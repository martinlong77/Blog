...
<title><?php echo ($info["nickname"]); ?>的关注_果冻记</title>
</head>
<body>
<div class="main">
    ...
    <div class="content_resize"><div class="content_resize2">
      <div class="mainbar">
        <div class="article">
		  ...
		  <?php if(is_array($followList)): $i = 0; $__LIST__ = $followList;if( count($__LIST__)==0 ) : echo "暂时还没有关注" ;else: foreach($__LIST__ as $key=>$follow): $mod = ($i % 2 );++$i;?><p class="relation_list" id="id_<?php echo ($follow["follow_id"]); ?>"><a href="<?php echo U('Blog/problog?blog=1&id='.$follow['follow_id']);?>" target="_blank"><img src="<?php echo ($follow["avatar"]); ?>" width="68" height="68" title="<?php echo ($follow["nickname"]); ?>" alt="<?php echo ($follow["nickname"]); ?>" /></a><a href="<?php echo U('Blog/problog?blog=1&id='.$follow['follow_id']);?>" target="_blank"><span class="detail_name" title="<?php echo ($follow["nickname"]); ?>"><?php echo ($follow["cut_nickname"]); ?></span></a><a href="<?php echo U('Profile/followlist?follow=1&id='.$follow['follow_id']);?>"><span class="category">关注:<?php echo ($follow["following"]); ?></span></a> &nbsp;|&nbsp; <a href="<?php echo U('Profile/fanslist?id='.$follow['follow_id']);?>"><span class="category">粉丝:<?php echo ($follow["followers"]); ?></span></a><br />
		  <?php if(($_GET['id']) == $_SESSION['id']): ?><a href="javascript:;" onClick="move_cg(this)" pf_id="<?php echo ($follow["follow_id"]); ?>" cg_id="<?php echo ($follow["group_id"]); ?>"><span class="cate_sel">设置分组</span></a> &nbsp;|&nbsp; <a href="javascript:;" onClick="unfollow(this)" fid="<?php echo ($follow["follow_id"]); ?>"><span class="info" onMouseOver="on_unfollow(this)" onMouseOut="out_unfollow(this)">正在关注</span></a><br /><?php endif; ?>
		  简介:<span class="info" title="<?php echo ($follow["intro"]); ?>"><?php echo ($follow["cut_intro"]); ?></span>
    	  </p><?php endforeach; endif; else: echo "暂时还没有关注" ;endif; ?>		       
          <div class="clr"></div>
          </div>
   ...
   $.post('<?php echo U('Profile/rela?set=reFriend');?>',
	...
	function(){
     $('#now_fri').hide();
	  ...
	  <?php if(($_GET['id']) == $_SESSION['id']): ?><div class="gadget">
          <h2>关注分组</h2>
          <div class="clr"></div>
          <ul class="ex_menu" id="fg_list">
		  <?php if(is_array($cate_group)): $i = 0; $__LIST__ = $cate_group;if( count($__LIST__)==0 ) : echo "您还没有分组" ;else: foreach($__LIST__ as $key=>$cg): $mod = ($i % 2 );++$i; if(($_GET['gid']) == $cg["cg_id"]): ?><li id="cg_id_<?php echo ($_GET["gid"]); ?>"><a href="<?php echo U('Profile/followlist?follow=1&id='.$_GET['id']);?>"><span class="apply"><?php echo ($cg["name"]); ?></span></a></li>
		  <?php else: ?>
		  <li id="cg_id_<?php echo ($cg["cg_id"]); ?>"><a href="<?php echo U('Profile/followlist?follow=1&id='.$_GET['id'].'&gid='.$cg['cg_id']);?>"><span class="agree"><?php echo ($cg["name"]); ?></span></a></li><?php endif; ?>
		  <span class="spec"></span><?php endforeach; endif; else: echo "您还没有分组" ;endif; ?>
		  </ul>
		 <p></p>
