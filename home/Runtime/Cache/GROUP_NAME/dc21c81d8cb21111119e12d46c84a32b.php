...
<title><?php echo ($info["nickname"]); ?>_果冻写</title>
</head>
<body>
<div class="main">
  <div class="header">
...
function clearText(field)
{
  ...
  <div class="content">
    <div class="content_resize"><div class="content_resize2">
      <div class="mainbar">
        <div class="article">
		<?php if(empty($_GET['cid'])): if(!empty($top)): ?><div><h2><span style="color:#CC6666">置顶：</span><a href="<?php echo U('Blog/Page?blog=1&id='.$top['post_id']);?>" target="_blank"><?php echo ($top["post_title"]); ?></a> <span><?php if(!empty($top["reproduced"])): ?>转载时间:<?php endif; ?>
		  (<?php echo (date('Y-m-d H:i',$top["post_createtime"])); ?>)</span></h2>
		    <div class="clr"></div>
          <p><span class="tag">标签：</span> <?php if(is_array($top["post_tag"])): foreach($top["post_tag"] as $key=>$sub): ?><a href="<?php echo U('Pub/search?search='.$sub);?>"><?php echo ($sub); ?></a>&nbsp;&nbsp;<?php endforeach; endif; ?></p>
          <p><?php echo ($top["post_content"]); ?></p>
          <p class="spec"><span>(<?php echo ($top["readcount"]); ?>) 阅读</span> &nbsp;|&nbsp; <?php if(in_array(($top["post_id"]), is_array($_SESSION["collectId"])?$_SESSION["collectId"]:explode(',',$_SESSION["collectId"]))): ?><a href="javascript:;" onClick="uncol_art(this)" aid="<?php echo ($_GET["id"]); ?>" pid="<?php echo ($top["post_id"]); ?>">取消收藏</a><?php else: ?>
          <a href="javascript:;" onClick="col_art(this)" aid="<?php echo ($_GET["id"]); ?>" pid="<?php echo ($top["post_id"]); ?>">收藏</a><?php endif; ?> &nbsp;|&nbsp; 
		  <?php if(($_GET['id']) == $_SESSION['id']): if(empty($top["reproduced"])): ?><a href="<?php echo U('Blog/editArticle?blog=1&id='.$top['post_id'].'&uid='.$_GET['id']);?>">编辑</a> &nbsp;|&nbsp;<?php endif; ?><a href="javascript:;" onClick="del_art(this)" jump="1" pid="<?php echo ($top["post_id"]); ?>" uid="<?php echo ($_GET["id"]); ?>" name="<?php echo ($top["post_title"]); ?>">删除</a> &nbsp;|&nbsp; <a href="javascript:;" onClick="art_untop()">取消置顶</a> &nbsp;|&nbsp; <a href="javascript:;" onClick="move_cg(this)" id="pf_id_<?php echo ($top["post_id"]); ?>" cg_id="<?php echo ($top["post_category"]); ?>" pf_id="<?php echo ($top["post_id"]); ?>">修改分类</a><?php else: ?><a href="<?php echo U('Blog/reproduced?id='.$top['post_id']);?>">举报</a><?php endif; ?>&nbsp;|&nbsp; <a href="<?php echo U('Blog/Page?blog=1&id='.$top['post_id']);?>" target="_blank">查看全文&raquo;</a>
		  <a href="<?php echo U('Blog/Page?blog=1&id='.$top['post_id']);?>" class="com" target="_blank"><span><?php echo ($top["comcount"]); ?> 评论</span></a></p></div><?php endif; ?>		  		 
		<?php if(($postList == NULL) AND ($top == NULL)): ?><span class="info"><?php if(($_GET['id']) == $_SESSION['id']): ?>您还没有写过博客哦，马上<a href="<?php echo U('blog/newblog?new=1');?>">写一篇</a>记录一下生活吧~<?php else: ?>博主暂时还没有写过博客哦<?php endif; ?></span><?php else: ?>
		  <div id="ajax_content"> 		  
		  		<?php if(is_array($postList)): $i = 0; $__LIST__ = $postList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><div id="id_<?php echo ($m["post_id"]); ?>"><h2><a href="<?php echo U('Blog/Page?blog=1&id='.$m['post_id']);?>" target="_blank"><?php echo ($m["post_title"]); ?></a> <span><?php if(!empty($m["reproduced"])): ?>转载时间：<?php endif; ?>
		    (<?php echo (date('Y-m-d H:i',$m["post_createtime"])); ?>)</span></h2>
		  <div class="clr"></div>
          <p><span class="tag">标签：</span> <?php if(is_array($m["post_tag"])): foreach($m["post_tag"] as $key=>$sub): ?><a href="<?php echo U('Pub/search?search='.$sub);?>"><?php echo ($sub); ?></a>&nbsp;&nbsp;<?php endforeach; endif; ?></p>
          <p><?php echo ($m["post_content"]); ?></p>
          <p class="spec"><span>(<?php echo ($m["readcount"]); ?>) 阅读</span> &nbsp;|&nbsp; <?php if(($_SESSION['isLoginIn']) == "1"): if(in_array(($m["post_id"]), is_array($_SESSION["collectId"])?$_SESSION["collectId"]:explode(',',$_SESSION["collectId"]))): ?><a href="javascript:;" onClick="uncol_art(this)" aid="<?php echo ($_GET["id"]); ?>" pid="<?php echo ($m["post_id"]); ?>">取消收藏</a><?php else: ?> <a href="javascript:;" onClick="col_art(this)" aid="<?php echo ($_GET["id"]); ?>" pid="<?php echo ($m["post_id"]); ?>">收藏</a><?php endif; ?> &nbsp;|&nbsp; 
		  <?php if(($_GET['id']) == $_SESSION['id']): if(empty($m["reproduced"])): ?><a href="<?php echo U('Blog/editArticle?blog=1&id='.$m['post_id']);?>">编辑</a> &nbsp;|&nbsp; <a href="javascript:;" onClick="art_top(this)" pid="<?php echo ($m["post_id"]); ?>">置顶</a> &nbsp;|&nbsp; <a href="javascript:;"  onclick="move_cg(this)" id="pf_id_<?php echo ($m["post_id"]); ?>" cg_id="<?php echo ($m["post_category"]); ?>" pf_id="<?php echo ($m["post_id"]); ?>">修改分类</a> &nbsp;|&nbsp;<?php endif; ?>
		  <a href="javascript:;" onClick="del_art(this)" pid="<?php echo ($m["post_id"]); ?>" uid="<?php echo ($_GET["id"]); ?>" name="<?php echo ($m["post_title"]); ?>">删除</a>
		  <?php else: ?>
		  <a href="<?php echo U('Blog/reproduced?id='.$m['post_id']);?>">举报</a><?php endif; ?> &nbsp;|&nbsp;<?php endif; ?><a href="<?php echo U('Blog/Page?blog=1&id='.$m['post_id']);?>" target="_blank">查看全文&raquo;</a>
		  <a href="<?php echo U('Blog/Page?blog=1&id='.$m['post_id']);?>" target="_blank" class="com"><span><?php echo ($m["comcount"]); ?> 评论</span></a></p></div><?php endforeach; endif; else: echo "" ;endif; ?>
          <div class="clr"></div>
        <p class="pages" id="page"><small>&nbsp;&nbsp;&nbsp;</small><?php echo ($page); ?></p>		
		  </div>
		  <?php if(($_SESSION["id"]) == $_GET["id"]): ?><div id="delete_confirm" title="删除确认">
...
function del_art(param){	
  ...
  var jump = '<?php echo ($navi["jump"]); ?>';
  var url = '<?php echo U('Blog/deleteArticle');?>';
  $('#pid').val(pid);
...
function art_top(param){
 ...
 $.post('<?php echo U('Blog/setTop?set=top');?>',
  	{pid:pid},
        function(data){
...
//取消置顶
function art_untop(){
 $.post('<?php echo U('Blog/setTop?set=untop');?>',
       function(data){
		$('#move_result').text(data.info);
		$('#move_result').dialog('open');
		setTimeout(function(){location.reload()},1000);	
			...
			<script type="text/javascript">
//转载
function reproduced(param){
 var pid = $(param).attr('pid');
 $.post('<?php echo U('Blog/reproduced');?>',
    {pid:pid},
    function(data){
	 $('#move_result').text(data.info);
	 $('#move_result').dialog('open');
  	 setTimeout(function(){location.reload()},1000);	
		  ...
		  <?php else: ?>
		  <?php if(empty($postList)): ?><font size="+1">这个分类下暂时还没有文章。</font>
		   <?php else: ?>
		  <div id="ajax_content"> 		  
		  		<?php if(is_array($postList)): $i = 0; $__LIST__ = $postList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><div id="id_<?php echo ($m["post_id"]); ?>"><h2><a href="<?php echo U('Blog/Page?blog=1&id='.$m['post_id']);?>" target="_blank"><?php echo ($m["post_title"]); ?></a> <span><?php if(!empty($m["reproduced"])): ?>转载时间：<?php endif; ?>
		    (<?php echo (date('Y-m-d H:i',$m["post_createtime"])); ?>)</span></h2>
     ...
     $('#now_fri').hide();
	   $('#add_fri').show();
		...
		<div class="gadget">
          ...
          <div class="clr"></div>
          <ul class="ex_menu" id="category_list">
		  <?php if(is_array($cateList)): $i = 0; $__LIST__ = $cateList;if( count($__LIST__)==0 ) : echo "暂时还没有分类" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i; if(($c["category_id"]) == $_GET['cid']): ?><li id="cg_id_<?php echo ($_GET["aid"]); ?>"><a href="<?php echo U('Blog/proBlog?blog=1&id='.$c['user']);?>"><span class="apply"><?php echo ($c["category_name"]); ?></span></a></li>
		  <?php else: ?>
            <li id="cg_id_<?php echo ($c["category_id"]); ?>"><a href="<?php echo U('Blog/proBlog?blog=1&id='.$c['user'].'&cid='.$c['category_id']);?>"><span class="agree"><?php echo ($c["category_name"]); ?></span></a></li><?php endif; endforeach; endif; else: echo "暂时还没有分类" ;endif; ?>
			</ul>
			<p></p>
