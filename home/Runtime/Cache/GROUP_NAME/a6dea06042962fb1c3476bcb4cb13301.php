...
<title><?php echo ($info["post_title"]); ?>_来自<?php echo ($info["nickname"]); ?>的文章_果冻写</title>
<meta http-equiv="description" content="<?php echo ($info["post_synopsis"]); ?>" />
<meta http-equiv="keywords" content="<?php if(is_array($info["post_tag"])): foreach($info["post_tag"] as $key=>$tag): echo ($tag); ?>,<?php endforeach; endif; ?>" />
</head>
<body>
<div class="main">
  <div class="header">
  ...
  <div class="content">
    <div class="content_resize"><div class="content_resize2">
      <div class="mainbar">
        <div class="article">
		<div class="con_img">
          <span><h2><?php echo ($info["post_title"]); ?> &nbsp;<span><?php if(!empty($info["reproduced"])): ?>转载时间：<?php endif; ?>(<?php echo (date('Y-m-d H:i',$info["post_createtime"])); ?>)</span></h2> <?php if(!empty($_SESSION['isLoginIn'])): if(empty($info["post_status"])): if(empty($info["reproduced"])): if(($info["user"]) == $_SESSION['id']): else: ?><span class="art_rep"><a href="javascript:;" onclick="reproduced(this)" pid="<?php echo ($info["post_id"]); ?>">＋转载</a></span><?php endif; endif; endif; endif; ?></span>
          <div class="clr"></div>
          <p><span class="tag">标签: </span>
            <?php if(is_array($info["post_tag"])): foreach($info["post_tag"] as $key=>$tag): ?><a href="<?php echo U('Pub/search?search='.$tag);?>"><?php echo ($tag); ?></a>&nbsp;&nbsp;<?php endforeach; endif; ?></p>
          <p><?php echo ($info["post_content"]); ?></p>         
          <p> <font color="#999999"><?php echo ($info["readcount"]); ?></font> <strong>阅读</strong> <span>&nbsp;&bull;&nbsp;<font color="#999999"><?php echo ($info["comcount"]); ?></font> <strong>评论</strong> </span>  <?php if(!empty($_SESSION['isLoginIn'])): ?><span>&nbsp;&bull;&nbsp;</span> <?php if(empty($info["post_status"])): if(empty($info["reproduced"])): if(($info["user"]) == $_SESSION['id']): ?><font color="#999999"><?php echo ($info["reproduced_count"]); ?></font> <strong>转载</strong><?php else: ?><font color="#999999"><?php echo ($info["reproduced_count"]); ?></font> <a href="javascript:;" onclick="reproduced(this)" pid="<?php echo ($info["post_id"]); ?>"><strong>转载</strong></a><?php endif; else: ?>转载<?php endif; else: ?>禁止转载<?php endif; ?> <span>&nbsp;&bull;&nbsp;</span> 
            <?php if(in_array(($info["post_id"]), is_array($_SESSION["collectId"])?$_SESSION["collectId"]:explode(',',$_SESSION["collectId"]))): ?><a href="javascript:;" onclick="uncol_art(this)" aid="<?php echo ($info["user"]); ?>" pid="<?php echo ($info["post_id"]); ?>">取消收藏</a><?php else: ?> <a href="javascript:;" onclick="col_art(this)" aid="<?php echo ($info["user"]); ?>" pid="<?php echo ($info["post_id"]); ?>">收藏</a><?php endif; endif; ?> 
		  
            <?php switch($info["user"]): case $_SESSION['id']: if(empty($info["reproduced"])): ?><span>&nbsp;&bull;&nbsp;</span> <a href="<?php echo U('Blog/editArticle?blog=1&id='.$info['post_id']);?>">编辑</a><?php endif; ?>&nbsp;&bull;&nbsp; <a href="javascript:;" onclick="del_art(this)" pid="<?php echo ($info["post_id"]); ?>" uid="<?php echo ($info["user"]); ?>" name="<?php echo ($info["post_title"]); ?>" >删除</a><?php break; default: ?>
			<?php if(!empty($_SESSION['isLoginIn'])): ?><span>&nbsp;&bull;&nbsp;</span> <a href="<?php echo U('Blog/reproduced?id='.$m['post_id']);?>">举报</a><?php endif; endswitch;?>
			</p>
		  
		  <!-- 上一篇 -->
		  <p><span class="info">上一篇：<?php if(($info["prev"]) == ""): ?>没有了<?php else: ?><a href="<?php echo U('Blog/page?id='.$info['prev']['id']);?>"><strong><?php echo ($info["prev"]["title"]); ?></strong></a><?php endif; ?></span></p>
			
			<!-- 下一篇 -->
			<span class="info">下一篇：<?php if(($info["next"]) == ""): ?>没有了<?php else: ?><a href="<?php echo U('Blog/page?id='.$info['next']['id']);?>"><strong><?php echo ($info["next"]["title"]); ?></strong></a><?php endif; ?></span>
			<?php if(($_SESSION["id"]) == $info["user"]): ?><div id="delete_confirm" title="删除确认">
