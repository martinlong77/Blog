...
<title><?php echo ($info["nickname"]); ?>的好友_果冻记</title>
</head>
<body>
<div class="main">
  <div class="header">
    ...
    <div class="content_resize"><div class="content_resize2">
      <div class="mainbar">
        <div class="article">
		  ...
		  <p class="spec"></p>
          <div class="clr"></div>
		  <?php if(is_array($friendList)): $i = 0; $__LIST__ = $friendList;if( count($__LIST__)==0 ) : echo "您还没有好友" ;else: foreach($__LIST__ as $key=>$friend): $mod = ($i % 2 );++$i;?><p class="relation_list" id="id_<?php echo ($friend["friend_id"]); ?>"><a href="<?php echo U('Blog/problog?blog=1&id='.$friend['friend_id']);?>" target="_blank"><img src="<?php echo ($friend["avatar"]); ?>" width="68" height="68" title="<?php echo ($friend["nickname"]); ?>" alt="<?php echo ($friend["nickname"]); ?>" /></a><a href="<?php echo U('Blog/problog?blog=1&id='.$friend['friend_id']);?>" target="_blank"><span class="detail_name" title="<?php echo ($friend["nickname"]); ?>"><?php echo ($friend["cut_nickname"]); ?></span></a><a href="<?php echo U('Profile/followlist?follow=1&id='.$friend['friend_id']);?>"><span class="category">关注:<?php echo ($friend["following"]); ?></span></a> &nbsp;|&nbsp; <a href="<?php echo U('Profile/fanslist?id='.$friend['friend_id']);?>"><span class="category">粉丝:<?php echo ($friend["followers"]); ?></span></a><br />
		  <?php if(($_GET['id']) == $_SESSION['id']): ?><a href="javascript:;" onClick="move_cg(this)" pf_id="<?php echo ($friend["friend_id"]); ?>" cg_id="<?php echo ($friend["group_id"]); ?>"><span class="cate_sel">设置分组</span></a> &nbsp;|&nbsp; <a href="javascript:;" onClick="on_break(this)" fid="<?php echo ($friend["friend_id"]); ?>"><span class="info" onMouseMove="on_friend(this)" onMouseOut="out_friend(this)">我的好友</span></a><br /><?php endif; ?>
		  简介:<span class="info" title="<?php echo ($friend["intro"]); ?>"><?php echo ($friend["cut_intro"]); ?></span>    	  </li><?php endforeach; endif; else: echo "您还没有好友" ;endif; ?> 		       
        <div class="clr"></div>
        </div>
     ...
     $('#now_fri').hide();
	   $('#add_fri').show();
		...
		<?php if(($_GET['id']) == $_SESSION['id']): ?><div class="gadget">
          <h2>关注分组</h2>
          <div class="clr"></div>
          <ul class="ex_menu" id="friend_list">
		  <?php if(is_array($cate_group)): $i = 0; $__LIST__ = $cate_group;if( count($__LIST__)==0 ) : echo "您还没有分组" ;else: foreach($__LIST__ as $key=>$cg): $mod = ($i % 2 );++$i; if(($_GET['gid']) == $cg["cg_id"]): ?><li id="cg_id_<?php echo ($_GET["gid"]); ?>"><a href="<?php echo U('Profile/friendList?about=1&id='.$_GET['id']);?>"><span class="apply"><?php echo ($cg["name"]); ?></span></a></li>
		  <?php else: ?>
		  <li id="cg_id_<?php echo ($cg["cg_id"]); ?>"><a href="<?php echo U('Profile/friendList?about=1&id='.$_GET['id'].'&gid='.$cg['cg_id']);?>"><span class="agree"><?php echo ($cg["name"]); ?></span></a></li><?php endif; ?>
