...
<table id="KeywordList" class="easyui-datagrid" title="<?php echo ($currentpos); ?>" data-options="border:false,fit:true,fitColumns:true,rownumbers:true,singleSelect:true,url:'<?php echo U('Setting/keyword_list');?>',toolbar:KeywordToolbar,pagination:true">
<thead>
	<tr>
...
function Corekw(){
	 openUrl("<?php echo U('Setting/core_list');?>", '核心条件管理', 0);
}
...
function KeywordAdd(){
	...
	if(add || edit){ 
	 ...
	 openUrl("<?php echo U('Setting/keyword_operate');?>", '设置新条件', 0);
}

function KeywordShow(id){	//首页展示
	$.messager.confirm('提示信息', '修改首页显示,确定吗？', function(result){
		if(!result) return false;		
		$.post("<?php echo U('Setting/keyword_show');?>", {id: id}, function(data){
			data.status ? openUrl("<?php echo U('Setting/keyword_list');?>", '搜索条件设置', 0) : $.messager.alert('提示信息', data.info, 'error');
		}, 'json');
	});
}
...
function KeywordEdit(id){	
	...
	var url = "<?php echo U('Setting/keyword_operate');?>";
...
function KeywordDelete(id){
	$.messager.confirm('提示信息', '确定要删除吗？', function(result){
		if(!result) return false;
		$.post("<?php echo U('Setting/delete_kw');?>", {id: id}, function(data){
			if(!data.status){
				$.messager.alert('提示信息', data.info, 'error');
			}else{				
				$.messager.alert('提示信息', data.info, 'info');
				openUrl("<?php echo U('Setting/keyword_list');?>", '搜索条件设置', 0);
			}
		}, 'json');
	});
}
