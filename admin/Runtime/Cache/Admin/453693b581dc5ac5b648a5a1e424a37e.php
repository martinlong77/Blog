...
<table id="AdminList" class="easyui-datagrid" title="<?php echo ($currentpos); ?>" data-options="border:false,fit:true,fitColumns:true,rownumbers:true,singleSelect:true,url:'<?php echo U('Admin/index');?>',toolbar:AdminToolbar,">
<thead>
	<tr>
		...
		<th data-options="field:'lastlogintime',width:15,formatter:AdminTimeText,sortable:true">最后登录时间</th>
...
<!-- 添加管理员 -->
<div id="AdminAddBox" class="easyui-dialog" title="添加管理员" data-options="modal:true,closed:true,iconCls:'icon-add',buttons:[{text:'确定',iconCls:'icon-ok',handler:function(){$('#AdminAddForm').submit();}},{text:'取消',iconCls:'icon-cancel',handler:function(){$('#AdminAddBox').dialog('close');}}]" style="width:480px;height:300px;"></div>

<!-- 编辑管理员 -->
<div id="AdminEditBox" class="easyui-dialog" title="编辑管理员" data-options="modal:true,closed:true,iconCls:'icon-edit',buttons:[{text:'确定',iconCls:'icon-ok',handler:function(){$('#AdminEditForm').submit();}},{text:'取消',iconCls:'icon-cancel',handler:function(){$('#AdminEditBox').dialog('close');}}]" style="width:480px;height:300px;"></div>
...
//添加
function AdminAdd(){
	$('#AdminAddBox').dialog({href:'<?php echo U('Admin/add');?>'});
