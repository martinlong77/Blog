...
class Mail{
	...
	public function quickSend($to,$subject,$msgbody,$smtp_user,$smtp_pass,$smtp_host='',$smtp_port=25,$smtp_tls=false,$helo=''){
		$to = trim($to);
		...
		$this->_post = array_merge($this->_post, array("from"=>$smtp_user, "smtp_username"=>$username, "smtp_password"=>$smtp_pass, "smtp_host"=>$smtp_host, "smtp_port"=>$smtp_port, 'to'=>$to,'subject'=>$subject,'content'=>$msgbody, 'tls'=>$smtp_tls, 'helo'=>$helo));
		return $this->send();
	}
	public function send(){
		if ( empty($this->_post['from']) 
			...
			$this->errmsg = "parameters from, to, subject, smtp_host, smtp_username, smtp_password can no be empty";
			return false;
		}
		...
		if ( substr($lastmessage,0,3) != 220 ){
			$this->errmsg = "error1:".$lastmessage.__LINE__;
			return false;
		}

		//HELO
		$yourname = empty($this->_post['helo']) ? 'www.phpol.net' : $this->_post['helo'];
		$lastact="EHLO ".$yourname."\r\n";

		fputs($fp, $lastact);
		$lastmessage == fgets($fp,512);
		if (substr($lastmessage,0,3) != 220 ){
			$this->errmsg = "error2:$lastmessage".__LINE__;
			return false;
		}
		while (true) {
			$lastmessage = fgets($fp,512);
		...
		if (substr($lastmessage,0,3) != 334){
			$this->errmsg = "error3:$lastmessage".__LINE__;
			return false;
		}
		//用户姓名
		...
		$lastmessage = fgets ($fp,512);
		if (substr($lastmessage,0,3) != 334){
			$this->errmsg = "error4:$lastmessage".__LINE__;
			return false;
		}
		//用户密码
		...
		if (substr($lastmessage,0,3) != "235"){
			$this->errmsg = "error5:$lastmessage".__LINE__;
			return false;
		}
		...
		if (substr($lastmessage,0,3) != 250){
			$this->errmsg = "error6:$lastmessage".__LINE__;
			return false;
		}
		...
		$lastmessage = fgets ($fp,512);
		if (substr($lastmessage,0,3) != 250){
			$this->errmsg = "error7:$lastmessage".__LINE__;
			return false;
		}
		...
		if (substr($lastmessage,0,3) != 354){
			$this->errmsg = "error8:$lastmessage".__LINE__;
			return false;
		}
	...
	public function setOpt($options){
		if (isset($options['subject']) && strlen($options['subject']) > self::subject_limitsize) {
			$this->errmsg = "subject size cannot larger than ".self::subject_limitsize." bytes";
			return false;
		}
